...
*/
		...
		function drawNode(node, p) {
			if( !node.shape )
			{
				...
				var titleText = document.createTextNode( node.data.keys.file );
...
/*
				if( node.isselected )
				{
					circle.orig_stroke = 'red';
					circle.setAttribute('stroke', 'red');
				}
				if( node.islvl1 )
				{
					circle.orig_stroke = 'orange';
					circle.setAttribute('stroke', 'orange');
				}
*/
				//node.shape.damelem = node.damelem;
...
*/
/*
				if( node.damelem.keys.get('type') == 'user' )
					node.shape.attr('stroke', 'blue');
				if( node.damelem.keys.get('type') == 'project' )
					node.shape.attr('fill', 'red');
				if( node.damelem.keys.get('type') == 'asset' )
					node.shape.attr('fill', 'orange');
				if( node.damelem.keys.get('type') == 'asset' )
					node.shape.attr('fill', 'orange');
				if( node.damelem.keys.get('type') == 'set' )
					node.shape.attr('fill', 'lightgreen');
*/
...
springy_damas.get_springy_graph = function( id ) {
	graph = new Springy.Graph();
	nodes = {};
	//alert(','.split(id));
	//alert(typeof(id.split(",")));
	damas.utils.command_a( { cmd: 'graph', id: id }, function( ret ){
		var json = JSON.parse(ret.text);
		$A( json['nodes']).each( function(n){
			//if(n.id != id)
			//{
			var elem = damas.utils.readJSONElement(n);
			nodes[elem.id] = graph.newNode( { 'label': elem.label() } );
			nodes[elem.id].damelem = elem; 
			//}
			//if( n.id == id )
			if( id.split(",").indexOf(n.id) !== -1 )
				nodes[elem.id].isselected = true;
		});
		$A( json['links']).each( function(l){
			if(l['src_id'] == id)
			{
				nodes[l['tgt_id']].islvl1 = true;
				//return;
			}
			if(l['tgt_id'] == id)
				nodes[l['src_id']].islvl1 = true;
				//return;
			if(l['tgt_id'] != id && l['src_id'] != id )
				graph.newEdge( nodes[l['tgt_id']], nodes[l['src_id']] );
...
//springy_damas.circle()



springy_damas.search_and_draw = function ( keys )
{
	damas.search(keys, undefined, undefined, 300, function(indexes){
		springy_damas.graph_all_nodes = springy_damas.graph_all_nodes.concat(indexes);
		damas.read(indexes, function(r){
			r.each( function(elem){
				if(nodes[elem.id] == undefined)
				{
					nodes[elem.id] = graph.newNode( { 'label': elem.label() } );
					nodes[elem.id].damelem = elem;
				}
		...
		damas.utils.command_a({cmd:'links2', id: springy_damas.graph_all_nodes.join(',')}, function(r){
			...
			$A(json).each(function(n){
				graph.newEdge( nodes[n[1]], nodes[n[2]]);
			});
		});
	});
}
